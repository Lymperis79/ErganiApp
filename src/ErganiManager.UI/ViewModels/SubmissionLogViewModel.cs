...
public partial class SubmissionLogViewModel : ViewModelBase, IAdminSectionViewModel
{
    ...
    private async Task LoadAsync()
    {
        ...
        try
        {
            ...
            if (config == null) { StatusMessage = "Database not configured."; return; }

            var opts = new DbContextOptionsBuilder<AppDbContext>();
            DbProviderFactory.Configure(opts, config);
            await using var db = new AppDbContext(opts.Options);

            var fromDt = FilterFrom.UtcDateTime;
            var toDt   = FilterTo.UtcDateTime;
